...
void writeInstanceCmpgt(ivl_lpm_t cmpgtLpm, int instNo, bool isEQ)
{
  ...
  const char *wireGT = NULL;
  const char *wireLT = NULL;
  const char *wireAnot = NULL;
  const char *wireBnot = NULL;
  const char *wireGTEQ = "cmpgtGTEQ";
  const char *wireLTEQ = "cmpgeLTEQ";
  const char *outPiName = NULL;
  const char *in0PiName = NULL;
  const char *in1PiName = NULL;
  const char *wireEQ = NULL;
  const char *wireOut = NULL;
  const char *wireEQand = NULL;
  const char *gateName = NULL;
  if (isEQ)
  {
    wireAnot = "cmpgeIN0not";
    wireBnot = "cmpgeIN1not";
    wireGT = "cmpgeGT";
    wireLT = "cmpgeLT";
    wireEQ = "cmpgeEQ";
    wireOut = "cmpgeout";
    wireEQand = "cmpgeEQand";
    gateName = "cmpge";
  }
  else
  {
    gateName = "cmpgt";
    wireAnot = "cmpgtIN0not";
    wireBnot = "cmpgtIN1not";
    wireGT = "cmpgtGT";
    wireLT = "cmpgtLT";
    wireEQ = "cmpgtEQ";
    wireOut = "cmpgtout";
    wireEQand = "cmpgtEQand";
  }
  ...
  if (lpmWidth > 1)
  {
    ...
    fprintf(fp, "wire [%d:0] %s%d, %s%d, ", lpmWidth-2, wireOut, instNo, wireEQand, instNo);
    if (isEQ)
      fprintf(fp, "%s%d;\n", wireLTEQ, instNo);
    else
      fprintf(fp, "%s%d;\n", wireGTEQ, instNo);
